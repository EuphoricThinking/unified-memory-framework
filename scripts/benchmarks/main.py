...
def main(directory, additional_env_vars, save_name, compare_names, filter):
    ...
    if filter:
        benchmarks = [
            benchmark for benchmark in benchmarks if filter.search(benchmark.name())
        ]
    ...
    for benchmark in benchmarks:
        try:
            ...
            processed: list[Result] = []
            for _ in range(options.stddev_validation_iterations):
                run_iterations(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--no-rebuild", help="Rebuild the benchmarks from scratch.", action="store_true"
    )
    parser.add_argument(
        "--env",
        type=str,
    ...
    parser.add_argument(
        "--stddev-threshold",
        type=float,
        help="If stddev % is above this threshold, rerun all iterations",
    ...
    parser.add_argument(
        "--stdev-validity-check-iterations",
        type=int,
        help="Max number of iterations of the loop, which calculates stddev after completed benchmark runs",
        default=options.stddev_validation_iterations,
    )
    ...
    options.stddev_validation_iterations = args.stdev_validity_check_iterations
